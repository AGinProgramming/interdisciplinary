...
from keras.layers import BatchNormalization #2/11/2020 - Kritika set all BN layers trainable=False
...
class BilinearUpsampling(Layer):
    ...
    def call(self, inputs):
        if self.upsampling:
            #KI changed tf.image to compat for tf2.0
...
def SepConv_BN(x, filters, prefix, stride=1, kernel_size=3, rate=1, depth_activation=False, epsilon=1e-3):
    ...
    x = BatchNormalization(name=prefix + '_depthwise_BN', epsilon=epsilon, trainable=False)(x)
    if depth_activation:
        x = Activation('relu')(x)
    x = Conv2D(filters, (1, 1), padding='same',
               use_bias=False, name=prefix + '_pointwise')(x)
    x = BatchNormalization(name=prefix + '_pointwise_BN', epsilon=epsilon, trainable=False)(x)
    if depth_activation:
        x = Activation('relu')(x)
...
def _xception_block(inputs, depth_list, prefix, skip_connection_type, stride,
    ...
    if skip_connection_type == 'conv':
        shortcut = _conv2d_same(inputs, depth_list[-1], prefix + '_shortcut',
                                kernel_size=1,
                                stride=stride)
        shortcut = BatchNormalization(name=prefix + '_shortcut_BN', trainable=False)(shortcut)
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, skip_connection, rate=1):
    ...
    if block_id:
        ...
        x = BatchNormalization(epsilon=1e-3, momentum=0.999,
                               name=prefix + 'expand_BN', trainable=False)(x)
    ...
    x = BatchNormalization(epsilon=1e-3, momentum=0.999,
                           name=prefix + 'depthwise_BN', trainable=False)(x)
    ...
    x = BatchNormalization(epsilon=1e-3, momentum=0.999,
                           name=prefix + 'project_BN', trainable=False)(x)
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='mobilenetv2', OS=16, alpha=1., activation=None):
    ...
    if backbone == 'xception':
        ...
        x = BatchNormalization(name='entry_flow_conv1_1_BN', trainable=False)(x)
        x = Activation('relu')(x)

        x = _conv2d_same(x, 64, 'entry_flow_conv1_2', kernel_size=3, stride=1)
        x = BatchNormalization(name='entry_flow_conv1_2_BN', trainable=False)(x)
        x = Activation('relu')(x)
        ...
        x = Activation(relu6, name='Conv_Relu6', trainable=False)(x)
    ...
    b4 = BatchNormalization(name='image_pooling_BN', epsilon=1e-5, trainable=False)(b4)
    ...
    b0 = BatchNormalization(name='aspp0_BN', epsilon=1e-5, trainable=False)(b0)
    ...
    x = BatchNormalization(name='concat_projection_BN', epsilon=1e-5, trainable=False)(x)
    x = Activation('relu')(x)
    x = Dropout(0.1)(x)
    # DeepLab v.3+ decoder
    ...
    if backbone == 'xception':
        ...
        dec_skip1 = BatchNormalization(
            name='feature_projection0_BN', epsilon=1e-5, trainable=False)(dec_skip1)
